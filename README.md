# Telegram Rewrite App — Тестовое задание

## Тестовое задание

Нужно сделать простой интерфейс на React + Python + Postgres, который позволяет:

- Подключиться к Telegram через Telethon.
- Забрать последние посты из Telegram-канала, где аккаунт состоит, но не является админом.
- Показать эти посты в интерфейсе.
- Выбрать пост и обработать его через короткий промпт.
- Получить рерайт текста.
- Опубликовать итоговый текст в другой Telegram-канал, где уже есть права на публикацию.
- Публикацию можно сделать любым удобным способом: через Telethon от имени аккаунта или через Telegram-бота, если бот добавлен в целевой канал с правами на публикацию.
- Видео поддерживать не нужно. Достаточно текста и, если успеете, изображений.

### Что важно

Интерфейс может быть простым, не нужно делать полноценный продуктовый дизайн. Но он должен быть аккуратным и понятным, не совсем техническая заглушка. Минимально в интерфейсе должны быть:

- поле для source-канала;
- поле для target-канала;
- список постов;
- кнопка Rewrite;
- поле с результатом рерайта;
- кнопка Publish;
- базовые статусы loading / success / error.
...
- .env.example.
- Краткое описание, как работает проект.

Главная цель — увидеть рабочую цепочку: забрали пост → обработали по промпту → опубликовали в другой канал.
...
Ключевое преимущество: приложение корректно забирает все картинки из Telegram-поста, включая посты-альбомы с несколькими изображениями, а не только первую картинку.
...
Список загружается понятными страницами по 10 постов: первая кнопка `Загрузить посты` берёт свежую страницу, кнопка `Подгрузить ещё 10 постов` добавляет более старые записи в конец списка. При этом frontend использует виртуализацию списка, поэтому даже при 200+ загруженных постах в DOM остаются только видимые строки.
...
### Производительность списка и media cache

Интерфейс сохраняет явную кнопочную пагинацию: оператор сам решает, когда подгрузить следующие 10 постов, без бесконечного скролла и неожиданных запросов. Сами строки списка рендерятся через виртуальный скролл, поэтому выбор поста и работа с большими выборками не пересоздают сотни DOM-элементов.

Картинки Telegram-постов скачиваются в файловое media storage один раз. При повторной загрузке уже обработанной страницы backend сначала проверяет наличие файла для `message_id` и не вызывает повторный `download_media`, если изображение уже лежит на диске.

Для Telegram-альбомов backend объединяет сообщения с одинаковым `grouped_id`, поэтому один пост с двумя и более картинками отображается и публикуется как единый пост со всеми изображениями.
...
- Ручные изображения проверяются по сигнатуре файла, whitelist расширений и лимиту `MEDIA_UPLOAD_MAX_BYTES`.
- Для HTTPS production включается `SESSION_COOKIE_SECURE=true`, чтобы session cookie уходила только по защищённому каналу.
